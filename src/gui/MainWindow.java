...
import projektas_skiplist.Book;
import projektas_skiplist.BooksGenerator;
...
public class MainWindow extends BorderPane implements EventHandler<ActionEvent> {
    ...
    private SL<Book> skipList;
    private int sizeOfInitialSubSet, sizeOfGenSet, colWidth, initialCapacity;
    private float loadFactor;
    private final BooksGenerator bookGenerator = new BooksGenerator();
    ...
    private void mapGeneration(String filePath) {
        ...
        if (filePath == null) {
            Book[] booksArray = bookGenerator.generateShuffle(sizeOfGenSet, sizeOfInitialSubSet, 1);
            for (Book c : booksArray) {
                skipList.add(
                        c.getBookRegNr(), //raktas
                        c);
            }
            KsGui.ounArgs(taEvents, MESSAGES.getString("mapPuts"), skipList.getSize());
        }
//        else { // Jei failas nurodytas skaitoma iš failo
//            skipList.load(filePath);
//            if (skipList.isEmpty()) {
...
//        }
        ...
        table.formTable(skipList.getSize() * 2 + 1, colWidth);
        String[][] modelList = skipList.getModelList();
        ...
        //IntStream.of(1, 3).forEach(p -> paneButtons.getButtons().getByKey(p).setDisable(false));
    }

    private void mapPut() {
//        Car car = bookGenerator.getCar();
//        skipList.put(
//                bookGenerator.getCarId(), // Raktas
//                car);
//        table.formTable(skipList.getMaxChainSize() * 2 + 1, colWidth);
//        String[][] modelList = skipList.getModelList(paneParam1.getTfOfTable().getByKey(5).getText());
//        table.setItems(FXCollections.observableArrayList(modelList));
...
//            String.valueOf(skipList.size()),
//            String.valueOf(skipList.getTableCapacity()),
//            String.valueOf(skipList.getMaxChainSize()),
//            String.valueOf(skipList.getRehashesCounter()),
//            String.valueOf(skipList.getLastUpdatedChain()),
//            // Užimtų maišos lentelės elementų skaičius procentais
//            String.format("%3.2f", (double) skipList.getChainsCounter() / skipList.getTableCapacity() * 100) + "%"
//        // .. naujus parametrus tęsiame čia ..
//        };
//        for (int i = 0; i < parameters.length; i++) {
//            String str = paneParam2.getTfOfTable().getByKey(i).getText();
//            if ((!str.equals(parameters[i]) && !str.equals("") && colorize)) {
//                paneParam2.getTfOfTable().getByKey(i).setStyle(Panels.STYLE_PINK);
//            } else {
//                paneParam2.getTfOfTable().getByKey(i).setStyle(Panels.STYLE_COMMON);
//            }
//            paneParam2.getTfOfTable().getByKey(i).setText(parameters[i]);
//        }
    }
